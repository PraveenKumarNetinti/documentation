...
<Note>
**Automatic Retries**: Failed webhook deliveries are automatically retried with exponential backoff (60s, 120s, 240s). This handles temporary network issues or endpoint downtime gracefully.
</Note>
...
<Info>
**Delivery Tracking**: Every webhook delivery is logged with full request/response details, timing metrics, and error messages. This makes debugging integration issues straightforward.
</Info>
...
<Note>
**Webhook Headers**: Each delivery includes metadata headers: signature for verification, event type for routing, delivery ID for idempotency, and attempt number for retry tracking.
</Note>
...
<Info>
**Exponential Backoff**: Retries use exponential backoff to avoid overwhelming failing endpoints. With 60s base delay: 1st retry after 60s, 2nd after 120s, 3rd after 240s.
</Info>
...
<Note>
**Event Broadcasting**: When you trigger an event, the system finds all active webhooks subscribed to that event type and queues deliveries asynchronously via Celery for non-blocking execution.
</Note>
