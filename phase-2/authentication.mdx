---
title: 'Authentication, Views, Services & Strategies'
description: 'Continuing with the authentication system implementation.'
sidebarTitle: "Authentication"
---

## Authentication App Implementation

<Info>
The authentication app handles all user identity, session management, and authentication strategies (password, magic link, OAuth).
</Info>
...
<Warning>
**Custom User Model**: This must be set before your first migration. Changing it later requires complex data migrations.
</Warning>
...
<Note>
**Token Rotation**: Refresh tokens are rotated on each use and tracked by family ID to detect token reuse attacks.
</Note>
...
<Note>
Services contain business logic separated from views. This makes code more testable and reusable across different interfaces (API, CLI, etc.).
</Note>
...
<Warning>
**Security Note**: Always return success even if the email doesn't exist to prevent email enumeration attacks.
</Warning>
...
<Info>
Celery tasks handle email sending asynchronously, preventing API requests from blocking while emails are sent.
</Info>
...
<Note>
Views are thin controllers that handle HTTP requests/responses. Business logic lives in services for better separation of concerns.
</Note>
...
<Info>
**Phase 2 Part 2 Complete!** You now have a fully functional authentication system with multiple strategies, session management, and email verification.
</Info>
