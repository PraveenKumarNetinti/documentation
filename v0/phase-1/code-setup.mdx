...
python_files = tests.py test_*.py *_tests.py  # Test file patterns
...
addopts = 
    ...
    --cov=apps  # Measure code coverage for apps/
...
Configuration for code quality tools (flake8, isort, mypy).
...
exclude =  # Directories to ignore
    ...
    */media/*,
    venv,
    env
ignore = E203, E266, W503  # Ignore specific rules (compatible with black)

[isort]
profile = black  # Use black-compatible import sorting
line_length = 100  # Match flake8 line length
skip = migrations, venv, env  # Skip these directories
...
plugins = mypy_django_plugin.main  # Django plugin for better type checking

[mypy.plugins.django-stubs]
django_settings_module = config.settings.development  # Django settings for mypy
```

### `manage.py`

Django's command-line utility for running management commands.
...
def main():
    ...
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings.development')  # Default to dev settings
    try:
    ...
    execute_from_command_line(sys.argv)  # Run the command
