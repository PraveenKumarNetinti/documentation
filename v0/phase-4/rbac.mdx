...
<Info>
**Permission Resolution**: The system collects permissions from all active roles assigned to a user, including inherited permissions, and caches the result for fast lookups.
</Info>
...
<Note>
**Wildcard Permissions**: The system supports wildcards like `members.*` (all member permissions) or `*.*` (superuser). This simplifies role configuration for broad access.
</Note>
